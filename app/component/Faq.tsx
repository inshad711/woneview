...
const AccordionItem: React.FC<AccordionItemProps> = ({ question, answer, isOpen, onClick }) => {
    return (
        <motion.div
            initial={false}
            className={`border border-black/10 rounded-[8px] overflow-hidden mb-2 transition-colors duration-300 ${isOpen ? 'bg-black/[0.03]' : 'bg-transparent hover:bg-black/[0.01]'
                }`}
        >
            <button
                onClick={onClick}
...
export default function Faq() {
    ...
    return (
        <div className='templateContainer'>
            <div className="bg-white text-black flex flex-col items-center py-20 px-4 md:px-8 selection:bg-[#db6e1a]/20">
                <motion.section
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
