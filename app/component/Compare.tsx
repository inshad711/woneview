            ...
            <div className="max-w-7xl mx-auto flex flex-col items-center">
                ...
                <div className="relative w-full  grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-6 items-stretch">
                    ...
                    <div className="rounded-[28px] px-4 py-6 bg-gradient-to-br from-[#F2F2F2] via-[#F2F2F2] to-white border border-gray-100 flex flex-col h-full">
                        <h5 className="text-xl font-semibold text-start text-[#0D0D0D] mb-5">Others</h5>
                        <div className="space-y-4 flex-grow">
                            {othersList.map((item, index) => (
                                <div key={index} className="flex items-center gap-4 rounded-xl border border-transparent">
                                    <XCircle className="text-gray-400 shrink-0" size={20} />
                                    <span className="text-gray-500 font-medium text-[15px] leading-relaxed">{item}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="rounded-[28px] px-6 py-6 bg-gradient-to-br from-[#284783] to-[#0B1421] text-white flex flex-col h-full">
                        <div className="space-y-4 mb-5">
                            ...
                            <div className="flex justify-start">
                                <div className="flex items-start ">
                                    <span className="text-xl font-bold tracking-tight">Elevar</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3 flex-grow">
                            {elevarList.map((item, index) => (
                                <div key={index} className="flex items-center gap-4 px-4 py-3 rounded-2xl bg-gradient-to-r from-white/10 to-transparent border border-white/5 backdrop-blur-sm ">
                                    <CheckCircle className="text-white shrink-0" size={20} />
                                    <span className="text-white font-medium text-[15px] leading-relaxed">{item}</span>
                                </div>
                            ))}
                        </div>
                    </div>
