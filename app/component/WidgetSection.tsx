...
type CategoryType = 'all' | 'ai-powered' | 'best-seller' | 'trending' | 'new' | 'social' | 'reviews';
                    ...
                    <aside className="hidden lg:block w-64 flex-shrink-0 sticky top-8 space-y-2">
                        {CATEGORIES.map((cat) => (
                            <button
                                key={cat.id}
                                onClick={() => setActiveCategory(cat.id)}
                                className={`w-full cursor-pointer flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 ${activeCategory === cat.id
